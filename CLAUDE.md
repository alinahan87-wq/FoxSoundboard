...
build step: `index.html`, `style.css`, `sounds.js`, `celebrate.js`, `colour.js`, `fox.js`, `numbers.js`, `scratch.js`, `bounce.js`, `blocks.js`, `circuit.js`, `app.js`, `sw.js`.
...
5. **Circuit:** games take `start({ onDone })`. When `onDone` is set, a win (always ending with the
   ...
   Free-play toys with no goal (like Bouncy ball and Building blocks) stay out of the circuit.
