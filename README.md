...
- **Seven activities.** Soundboard; Colour match (tap the four squares until they're all the same colour, then bubbles!); Spell FOX (tap the three letters until they read F-O-X); Count to 10 (tap the scattered numbers in order); Scratch & reveal (scratch away a pattern to find an animal, whose name is then shown and spoken); Bouncy ball (tap to bounce it, hold to power it up for bigger, sparklier bounces); and Building blocks (drag and stack blocks that fall with real gravity). Switch between them from the grown-up game menu. **Circuit** plays the four goal games in turn, moving on after each win.
